...
#include "kacanopen/driver/can_driver.h"
...
UNS8 canSend_driver(CAN_HANDLE fd0, Message const *m) {
  ...
  if (frame.can_id >= 0x800) frame.can_id |= CAN_EFF_FLAG;
  frame.can_dlc = m->len;
  if (m->rtr)
...
canOpen_driver(s_BOARD *board) {
  ...
  *(int *)fd0 = CAN_SOCKET(PF_CAN, SOCK_RAW, CAN_RAW);
  ...
  {
    int loopback = 1;
    err = CAN_SETSOCKOPT(*(int *)fd0, SOL_CAN_RAW, CAN_RAW_LOOPBACK, &loopback,
                         sizeof(loopback));
    if (err) {
  ...
  *baudrate = TranslateBaudRate(board->baudrate);
  if (!*baudrate) goto error_close;

  err = CAN_IOCTL(*(int *)fd0, SIOCSCANBAUDRATE, &ifr);
  if (err) {
    fprintf(stderr, "Setting baudrate %d failed: %s\n", *baudrate,
            strerror(CAN_ERRNO(err)));
    goto error_close;
  }
